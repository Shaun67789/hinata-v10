...
async function executeDeleteMsg() {
  ...
  alertBox(`Requesting deletion...`, 'info');
  
  try {
    const res = await fetch('/api/delete_msg', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    const data = await res.json();
    if (data.success) {
      alertBox(`Message deleted successfully!`, 'success');
      document.getElementById('admin-msg-url').value = '';
    } else {
       alertBox(`Error: ${data.error}`, 'danger');
    }
  } catch (e) {
