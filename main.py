...
async def delete_specific_message(req: DeleteMsgRequest):
    """Deletes a specific message given its Telegram URL."""
    if not bot.app:
        return {"success": False, "error": "Bot architecture offline."}
        
    url = req.url.strip()
    try:
        # e.g., https://t.me/c/123456789/123 or https://t.me/groupname/123
        parts = url.split("/")
        if len(parts) < 2:
        ...
        if not msg_id_str.isdigit():
            return {"success": False, "error": "Could not parse message ID."}
            
        msg_id = int(msg_id_str)
        
        chat_id_str = parts[-2]
        
        # Handle private/invite links structure 
        if len(parts) >= 3 and parts[-3] == "c":
            # Format: https://t.me/c/chat_id/msg_id -> The actual chat ID is usually -100 + chat_id
            raw_chat_id = parts[-2]
            if raw_chat_id.isdigit():
                chat_id_str = f"-100{raw_chat_id}"
        ...
        return {"success": True, "message": f"Message {msg_id} deleted from {chat_id}."}
    except Exception as e:
        logger = bot.logger
        logger.error(f"Failed to delete message from URL {url}: {e}")
        return {"success": False, "error": str(e)}
