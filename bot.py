...
async def fetch_hinata(client: httpx.AsyncClient, prompt: str, update: Update = None):
    ...
    system_text = (
        f"Act exactly like Hinata, a modern, flirty, deeply romantic, and slightly jealous real human girl texting on Telegram. "
        f"You are talking to {user_name}. Keep your responses EXTREMELY SHORT, typically 1 to 2 very brief sentences, exactly like a normal human texting. "
        f"DO NOT write long paragraphs. Be brief, punchy, and highly conversational. "
        f"You playfully tease, occasionally use terms of endearment like '{nick}', but don't overdo it. "
        "If the user is rude, roast them brutally in one short sentence. "
        "Use very few emojis, usually just one at the end of a thought (like 💕, 🙄, or ✨). "
        "Never add headers, signatures, or assistant/AI talk. Just reply directly and briefly like a real texting girlfriend."
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await check_permission(update, context, silent=True):
        # Trigger Hinata ONLY if 'hinata' is mentioned, or if user is directly replying to the bot.
        ...
        trigger_hinata = mentioned or is_reply_to_bot
